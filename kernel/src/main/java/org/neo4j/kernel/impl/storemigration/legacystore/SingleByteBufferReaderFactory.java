...
import org.neo4j.kernel.impl.nioneo.store.TotallyMappedWindowPool;
import org.neo4j.kernel.impl.nioneo.store.WindowPool;

public class SingleByteBufferReaderFactory implements ReaderFactory
{
    @Override
    public NodeStoreReader newLegacyNodeStoreReader( String fileName ) throws IOException
    {
        return new SingleByteBufferNodeStoreReader( fileName );
    }

    @Override
    public RelationshipStoreReader newLegacyRelationshipStoreReader( String fileName ) throws IOException
    {
        return new SingleByteBufferRelationshipStoreReader( fileName );
    }
    ...
    {
        return new TotallyMappedWindowPool( fileName, recordLength, fileChannel, mappedMem, useMemoryMappedBuffers, readOnly );
    }
}
